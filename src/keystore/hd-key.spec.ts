...
describe('HDKey', () => {
  ...
  it('signing', async () => {
    ...
    assert.isTrue(hdkey.verify(ma, a));
    assert.isTrue(hdkey.verify(mb, b));
    assert.isFalse(hdkey.verify(Buffer.alloc(32), Buffer.alloc(64)));
    assert.isFalse(hdkey.verify(ma, b));
    assert.isFalse(hdkey.verify(mb, a));
