...
export async function encrypt(data: Uint8Array, key: Uint8Array, iv: Uint8Array) {
  ...
  const encrypted = await crypto.subtle.encrypt({ name, iv, length }, await importKey(key), data);
...
export async function decrypt(data: Uint8Array, key: Uint8Array, iv: Uint8Array) {
  ...
  const decrypted = await crypto.subtle.decrypt({ name, iv, length }, await importKey(key), data);
